...
@section('content')
    ...
    <div class="scroll">
                            ...
                            <div class="card">
                                <div class="position-relative">
                                    <a href="Pages.Product.Detail.html">
                                        @if($product->images)
                                        <div class="side_view">
                                            <img src="{{productImages($product->images)}}" alt="Card image cap" class="card-img-top" style="height: 200px;" />
                                        </div>
                                        @else
                                            No avatar
                                        @endif
        ...
        <div class="chat-input-container d-flex justify-content-between align-items-center">
            <form action="{{ route('form.checkout') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" value="{{ $new_buyer->enkripsi_token }}" name="enkripsi">
                <input type="hidden" value="{{ $new_buyer['id'] }}" name="buyer_id">
                <input type="hidden" value="{{ $user['id'] }}" name="user_id">
                <button type="submit" class="btn btn-danger btn-block fw-bold" style="font-size: 20px;">Lihat Pesanan</button>
            </form>
        </div>
@endsection
